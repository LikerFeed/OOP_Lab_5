...
{
	parentShape = shape;
	IvanToolBar.OnTool(hWnd, lParam);
...
void MyEditor::OnMouseMove(HWND hWnd)
{
	if (xstart)
	{
		...
		if (xend) pshape[i]->DrawRubber(hdc);
...
{
	IvanToolBar.OnCreate(hWnd, hInst);
}

void MyEditor::OnNotify(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	IvanToolBar.OnNotify(hWnd, wParam, lParam);
}

void MyEditor::OnSize(HWND hWnd)
{
	IvanToolBar.OnSize(hWnd);
}